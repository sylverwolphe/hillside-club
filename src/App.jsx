...
import { C, SketchFilter, NavIcons, Modal, Toast } from "./theme";
...
function AuthModal() {
  ...
  return (
    <Modal open={showAuthModal} onClose={handleClose}>
...
function BottomNav() {
  ...
  const navItems = [
    ...
    { key: "account", path: "/profile", icon: NavIcons.account, label: user ? "You" : "Sign In" },
  ];

  const isActive = (item) => {
    if (item.key === "home") return path === "/" || path === "";
    if (item.key === "pinboard") return false;
    if (item.key === "account") return path === "/profile";
        ...
        return (
          ...
          >
            ...
            <span style={{ position: "relative" }}>
              {item.icon}
              {item.key === "account" && hasUnread && (
                <span style={{
                  position: "absolute", top: -2, right: -4, width: 8, height: 8,
                  borderRadius: "50%", background: C.terracotta,
